...
export async function resolveStoredSession(
  ...
  for (const profile of otherProfiles) {
    try {
      ...
      // Track a genuine 404 here too — a remembered id that's gone from every
      // profile should be forgotten, not re-probed on each launch.
      if (isSessionNotFoundError(err)) {
        sessionNotFound = true
      }
  ...
  // The id resolved on no profile and at least one backend answered a genuine
  // 404 (as opposed to a transient/network error). If it's the remembered
  // last-session id, forget it so the next launch starts clean instead of
  // re-404ing the same dead id forever.
  if (sessionNotFound) {
